    ...
    */
    ...
    async _checkFrenzy(event) {
        ...
        if (this.object.canRoll) {
            ...
            this.object.hasRolled = true;

            if (this.object.type == CONFIG.worldofdarkness.splat.vampire) {
                // V20 Self-Control: any success resists; zero successes means frenzy
                if (rollSuccesses > 0) {
                    await this._setFrenzyCondition(false);
                }
                else {
                    await this._setFrenzyCondition(true);
                }
                this.object.close = true;
                this.close();
            }
            else if (this.object.type == CONFIG.worldofdarkness.splat.werewolf) {
                // Keep accumulating across rolls; clear frenzy once required successes are met
    ...
    /**
     * Toggle the Frenzy condition on the actor (PC and legacy share system.conditions.isfrenzy).
     */
    async _setFrenzyCondition(isFrenzy) {
        if (this.actor.system.conditions?.isfrenzy === isFrenzy) {
            return;
        }

        await this.actor.update({
            "system.conditions.isfrenzy": isFrenzy
        });
    }
    ...
    async _closeForm(event) {
        ...
        // Closing after an incomplete werewolf frenzy check counts as entering frenzy
        if ((this.object.type == CONFIG.worldofdarkness.splat.werewolf) &&
            (this.object.hasRolled) &&
            (this.object.numSuccesses < this.object.successesRequired)) {
            await this._setFrenzyCondition(true);
        }
